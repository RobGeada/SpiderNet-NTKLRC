...
from spider_net.data_loaders import Cutout as CUTOUT
...
class LinearRegionCount(object):
    ...
    def update2D(self, activations):
        ...
        if self.activations is None:
            self.activations = torch.zeros(self.n_samples, n_neuron, device=torch.device("cuda"))
...
class Linear_Region_Collector:
    def __init__(self, models=[], input_size=(64, 3, 32, 32), sample_batch=100, dataset='cifar10', data_path='data/CIFAR10', seed=0):
        self.models = []
    ...
    def forward_sum(self, n_samples):
        out = 0
        for _ in range(n_samples):
            out += self.forward_batch_sample()[0]
            self.clear(perm=False)
        return out
